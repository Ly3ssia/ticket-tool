...
module.exports = {
    ...
    name: ['adannebaba'],                        //Komut İsmini Belirtiyor
    ...
    async execute(client, message, args) {          //Komut Handlerına Göre Tanımlama Yeri. Burayı Ellemeyin
        ...
        con.query(`SELECT * FROM 101m WHERE ADI="${adx} ${ad2x}" AND ANNEADI="${anneadıx}" AND BABAADI="${babaadıx}"`, function (err, result) {
              if (err) throw err;
            ...
            message.reply({  files: [ckt] })*/
              ...
              message.reply(`:tada: ${adx} ${anneadıx} ${babaadıx} isminde **${data.length}** kişi bulundu.`)
              let dosyahazırla = new Discord.MessageAttachment(Buffer.from(as31), `dracocheck.txt`);
              message.reply({ files: [ dosyahazırla ] })
              message.channel.send(`${message.author.tag} tarafından ${adx} ${anneadıx} ${babaadıx} buldun la hadi yine iyisin ;).`)
            }); 
